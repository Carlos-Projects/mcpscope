                            ...
                            "rules": [
                                {
                                    ...
                                    "fullDescription": {
                                        "text": "Full description of test rule"
                                    },
                                }
                            ],
                        }
...
class TestScanDiff:
    def test_diff(self):
        ...
        store = Store(db_path=db_path)
        store.save_scan(
            ScanRun(id="a", scanner="t", target="t1"),
            [
                Finding(
                    scan_id="a",
                    scanner="t",
        ...
        store.save_scan(
            ScanRun(id="b", scanner="t", target="t2"),
            [
                Finding(
                    scan_id="b",
                    scanner="t",
...
class TestWebhookPayload:
    def test_notify_skips_safe(self):
        ...
        notify_scan_imported(
            [], findings=findings, findings_count=1, critical_count=0, high_count=0
        )
...
class TestCSVReport:
    def test_csv_format(self):
        ...
        store = Store(db_path=db_path)
        store.save_scan(
            ScanRun(id="csv-test", scanner="t", target="t"),
            [
                Finding(
                    scan_id="csv-test",
                    scanner="t",
